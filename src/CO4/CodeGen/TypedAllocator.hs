...
fromKnownAllocator adt = do
  def <- mkDefinition
  return [ instanceD predicates instType [def] ]
  where
    allocType    = typeOfAdt adt
    typeVars     = adtTypeVariables adt

    predicates   = map (\v -> classP ''FromKnown [varT v]) typeVars
...
completeAllocator adt = [ instanceD predicates instType [def] ]
  where
    allocType  = typeOfAdt adt
    typeVars   = adtTypeVariables adt

    predicates = map (\v -> classP ''Complete [varT v]) typeVars

...
module CO4.EncodedAdt.Sequential
  ( EncodedAdt (..), EncodedConstructor, IntermediateAdt (..)
  , bottom, isBottom, flags, bottomConstructor, isBottomConstructor
  , constantConstructorIndex, constantConstructor
  , caseOf, encodedConstructor, constructorArgument
  , toIntermediateAdt
  )
where
...
constantConstructor :: (Primitive p) => EncodedAdt p -> Bool
constantConstructor = isJust . constantConstructorIndex

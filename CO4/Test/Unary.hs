...
$([d|   
      constraint a b = 
         foldl mtimes a [a] `mgreater` foldl mtimes a [b,a] && mvalid a && mvalid b
        -- foldl1 mtimes [a,a] `mgreater` foldl1 mtimes [a,b,a] && mvalid a && mvalid b

        -- mvalid (foldl1 mtimes [a,a])
        -- mvalid (mtimes a a)


        -- Raml: 5 (mtimes a a `mgreater` (mtimes a (mtimes b a))) && mvalid a && mvalid b
        
{-
        let a2 = mtimes a a
            b2 = mtimes b b
        in
        (mtimes a2 b2 `mgreater` (mtimes b (mtimes b2 (mtimes a2 a)))) && mvalid a && mvalid b
-}
      -- Matrices
...
testSimple = 
  let unknowns dim bits = 
	replicateM dim $ replicateM dim $ replicateM bits boolean :: SAT [[[Boolean]]]
      test [a,b] = runIdentity ( constraint a b  ) :: Bool
  in do
    mResult <- solve ( do [a,b] <- forM [1..2] $ const $ unknowns 3 5
                          r <- constraint a b
                          assert [r]
                          return ( decode [a,b] )
                   ) :: IO (Maybe [[[[Bool]]]])
